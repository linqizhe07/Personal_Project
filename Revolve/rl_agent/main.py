...
def train_u2o(
    ...
    """
    Fine-tune a pretrained U2O skill policy with a fixed z* and task reward.

    Instead of creating a fresh SAC, this loads the pretrained SAC model
    and continues training with the LLM-generated task reward.
    The env is already wrapped with SkillConditionedWrapper(fixed_z=z*).
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    current_timesteps = 0
    ...
    while current_timesteps < total_timesteps:
        ...
        model_save_path = os.path.join(
            model_checkpoint_path,
            f"SAC_u2o_{generation_id}_{counter}_{current_timesteps}.zip",
        )
        model.save(model_save_path)
        env.render()
...
def run_training_u2o(
    ...
    """
    U2O version of run_training.
    Wraps HumanoidEnv with SkillConditionedWrapper(fixed_z=z*),
    then loads pretrained SAC and fine-tunes with task reward.
    """
    from u2o.wrappers import SkillConditionedWrapper
    ...
    # Wrap with skill conditioning (fixed z*)
    gymenv = SkillConditionedWrapper(
        env=gymenv,
